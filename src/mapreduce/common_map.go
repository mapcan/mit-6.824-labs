...
func doMap(
    ...
    if err != nil {
        log.Print("Read input file: ", err)
    }

    mapResult := mapF(inFile, string(contents))
    kvHash := make(map[int][]KeyValue)
    for _, kv := range(mapResult) {
        key := kv.Key
        r := ihash(key) % nReduce
        kvHash[r] = append(kvHash[r], kv)
    }

    for r := 0; r < nReduce; r++ {
        intermediate := reduceName(jobName, mapTaskNumber, r)
        f, err := os.Create(intermediate)
        defer f.Close()
        if err != nil {
            log.Printf("Intermediate file %s cannot be created: %s", f, err)
        }
        kvs, ok := kvHash[r]
        if !ok {
            continue
        ...
        for _, kv := range(kvs) {
            err := enc.Encode(&kv)
            if err != nil {
                log.Print("Json kv encode error: ", err)
            }
        }
    }
}
